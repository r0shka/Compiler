    ...
    while ((c = getc(file)) != EOF){
		...
		isNumber=0;
            ...
            if(buffer[0]!=0){
				...
				strcpy(name[nameCounter++], buffer);
			if(isNumber){
				puts("NOMBRE");
		...
		initBuffer(buffer);
            ...
            if(buffer[0]!=0){
                printf("%s\n", buffer);//DATA STORAGE, FOR LATER USAGE
				if(!isNumber) strcpy(name[nameCounter++], buffer);
                if(isNumber){
                    puts("NOMBRE");
                    symbolesArray[j++]=NOMBRE;
                    isNumber=0;
            ...
            if(buffer[0]!=0){
                ...
                else if(!strcmp(buffer, "entier")){				
                    puts("ENTIER");
                    symbolesArray[j++]=ENTIER;
                    i=0;
                    initBuffer(buffer);
                }
                else if(!strcmp(buffer, "retour" )){
                    puts("RETOUR");
                    symbolesArray[j++]=RETOUR;
                    i=0;
                    initBuffer(buffer);
                }
                else if(!strcmp(buffer, "tantque" )){
                    puts("TANTQUE");
                    symbolesArray[j++]=TANTQUE;
                    i=0;
                    initBuffer(buffer);
                }
                else if(!strcmp(buffer, "faire" )){
                    puts("FAIRE");
                    symbolesArray[j++]=FAIRE;
                    i=0;
                    initBuffer(buffer);
                }
                else if(!strcmp(buffer, "alors" ) || !strcmp(buffer, "alors " )){
                    puts("ALORS");
                    symbolesArray[j++]=ALORS;
                    i=0;
                    initBuffer(buffer);
                }
                else if(!strcmp(buffer, "sinon" ) || !strcmp(buffer, "sinon " )){
                    puts("SINON");
                    symbolesArray[j++]=SINON;
                    i=0;
                    initBuffer(buffer);
            ...
            if(buffer[0]!=0){
                ...
                else if(!strcmp(buffer, "ecrire" ) || !strcmp(buffer, "ecrire " ) || !strcmp(buffer, " ecrire" )){
                    puts("ECRIRE");
